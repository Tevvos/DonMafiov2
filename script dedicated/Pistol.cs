...
public class Pistol : MonoBehaviour, IWeaponAmmo
{
    [Header("Firing")]
    [SerializeField] private float fireInterval = 0.20f; // 5 rps
    ...
    [Tooltip("Recharge auto quand le chargeur tombe à 0 (seulement s'il reste de la réserve).")]
    [SerializeField] private bool  autoReloadOnEmpty = false;

    [Header("Reserve (server only)")]
    [SerializeField, Tooltip("Nombre max de balles en réserve.")]
    private int maxReserve = 90;
    [SerializeField, Tooltip("Réserve initiale au spawn.")]
    private int initialReserve = 0;

    [Header("Camera Recoil")]
    [Range(0f, 2f)] [SerializeField] private float shootShakeIntensity = 0.4f;
    ...
    public bool ServerStartReload(NetworkRunner runner)
    {
        ...
        if (reserveAmmo <= 0) return false; // 🔑 empêche recharge infinie sans réserve
        isReloading = true;
    ...
    {
        fireInterval  = Mathf.Clamp(fireInterval, 0.05f, 2f);
        ...
        if (bulletPrefab.Equals(default(NetworkPrefabRef)))
            Debug.LogWarning($"[Pistol] '{name}' n'a pas de bulletPrefab assigné.");
    }
#endif
}
